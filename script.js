let availableFiles = [], currentFile = null;
let allQuestions = [], currentQuestions = [];
let currentMode = null, isChecked = false;
...
async function loadConfig() {
    try {
        ...
        showError(`<strong>Błąd konfiguracji!</strong><br>Sprawdź plik 'config.json'.<br><small>${error.message}</small>`);
    }
}
...
function renderFileSelector(semesters) {
    ...
    semesters.forEach(semester => {
        ...
        semester.files.forEach(file => {
            ...
            // Capture the current globalIndex in the closure
...
async function selectFile(index) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Nie można wczytać pliku: ${currentFile.file}`);
        allQuestions = await response.json();
        document.getElementById('loading').style.display = 'none';
        document.getElementById('mode-selector').style.display = 'block';
        
        document.getElementById('mode-selector').scrollIntoView({ behavior: 'smooth' });
...
function prepareQuiz() {
    ...
    switch(currentMode) {
        case 'study':
            ...
            description.textContent = 'Wszystkie pytania z poprawnymi odpowiedziami. Użyj CTRL+F, aby szybko wyszukać.';
        ...
        case 'fullquiz':
            ...
            description.textContent = 'Wszystkie pytania w trybie quizu. Pokaż co potrafisz!';
            currentQuestions = [...allQuestions];
            renderQuizMode();
            break;
    }
...
function checkAnswers() {
    ...
    currentQuestions.forEach((question, qIndex) => {
        ...
        const selectedAnswers = Array.from(checkboxes).filter(cb => cb.checked).map(cb => parseInt(cb.dataset.answer));

        if (arraysEqual(selectedAnswers.sort(), question.correct.sort())) correctQuestions++;
...
function showResults(correctQuestions) {
    ...
    if (percentage === 100) { grade = '🏆 Perfekcja!'; gradeColor = 'var(--color-correct)'; encouragement = 'Absolutne mistrzostwo! 🎉'; }
    else if (percentage >= 80) { grade = '🎉 Znakomity wynik!'; gradeColor = '#4A90E2'; encouragement = 'Świetna robota! 💪'; }
    else if (percentage >= 60) { grade = '👍 Dobry wynik!'; gradeColor = 'var(--color-missed)'; encouragement = 'Jesteś na dobrej drodze! 📈'; }
    else if (percentage >= 40) { grade = '📈 Warto powtórzyć'; gradeColor = '#F5A623'; encouragement = 'Następnym razem będzie lepiej! 🔄'; }
    else { grade = '📚 Czas na naukę'; gradeColor = 'var(--color-incorrect)'; encouragement = 'Nie poddawaj się! 🚀'; }
...
function drawNextRandomQuestions() {
    ...
    document.getElementById('quiz-description').textContent = `Oto kolejny zestaw ${questionCount} pytań. Powodzenia!`;
...
function backToMenu() {
    ['quiz-info', 'controls', 'legend'].forEach(id => document.getElementById(id).style.display = 'none');
    ['quiz-content', 'results'].forEach(id => document.getElementById(id).innerHTML = '');
    ['file-selector'].forEach(id => document.getElementById(id).style.display = 'block');
    document.getElementById('mode-selector').style.display = 'none';
    currentMode = null; isChecked = false;
...
function getModeDisplayName(mode) {
    const names = { study: 'Tryb Nauki', random5: 'Szybki Test', fullquiz: 'Pełny Egzamin' };
